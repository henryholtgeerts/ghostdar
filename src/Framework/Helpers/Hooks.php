...
namespace Ghostdar\Framework\Helpers;
	...
	public static function addAction( $tag, $class, $method = '__invoke', $priority = 10, $acceptedArgs = 1 ) {
		...
		add_action(
			...
			static function () use ( $tag, $class, $method ) {
				...
				if ( apply_filters( "ghostdar_disable_hook-{$tag}", false ) || apply_filters( "ghostdar_disable_hook-{$tag}:{$class}@{$method}", false ) ) {
					return;
				}

				$instance = ghostdar( $class );
